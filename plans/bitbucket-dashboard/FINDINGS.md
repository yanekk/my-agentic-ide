...
| Date | | Finding |
|---|---|---|
| 2026-09-05 | ✅ | T03 model hand-verified vs real `cribl` PRs (read-only key). Comment accessors correct: `inline` present on inline comments, `parent` on replies, a truthy `resolution` object on a resolved root (absent when open — PR#85 resolved vs #40 open), author `c.user.uuid`. PR-list author.nickname/uuid, participants, reviewers all present. No model fix. |
