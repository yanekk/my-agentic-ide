...
| 2026-09-04 *(T07 → T12)* | The reader launches with **`-colorscheme one-dark`**, on the **launch line** and never in the user's own micro config | micro's default draws the tab bar **light text on a light strip**: the user had three files open, could not see the bar at all and asked what “tabs” meant — *“white tabs over a black background, not white tabs on a white strip”*. There is no `tabbar` **option** to toggle — the bar's colours come from the colourscheme's own `tabbar` group, so a scheme is the only lever. Writing `~/.config/micro/settings.json` was rejected for the reason broot's verbs were: the cockpit does not edit the user's own config, so micro outside the cockpit is untouched and a settings file they write later is not fought over. The name must be one micro **ships** — an unknown one is a *startup* failure (`<name> is not a valid colorscheme`), not a silent fallback, and the 1 s healer would relaunch it forever |
...
- **A GUI-editor tab bar — a light active tab with a small `[x]` on each.** Asked for 2026-09-04 while judging T12's scheme, and **declined the same day by the user** once the cost was clear. micro's tab bar layout is not reachable from a colourscheme, an option or a plugin: the four tab-bar options (`tabreverse`, `tabhighlight`, `basename`, `tabsize`) recolour and relabel it, and none of them adds a close affordance. Getting one means either a different reader — and §2.2, §4 and T04–T06 are micro-specific throughout, from the push protocol to the detection to the heal — or the cockpit rendering its own tab bar above the reader, which it has the data for (`viewer-tabs.json`) but neither the screen row nor a way to route a click back into micro. Weigh also that a close button is a **mouse** affordance for what `Ctrl-Q` already does, against a mode whose flagship T07 result was that it can be driven without a mouse. The colours the request also asked for **are** delivered: micro's `tabreverse` is on by default, so a dark scheme gives a dark strip and a light block for the current tab.
