...
| 2026-08-26 | 📌 **Baseline test measurement**: `spikes/notes-test/run.sh` 39 assertions ALL PASS, `spikes/cockpit-test/run.sh` 82 assertions ALL PASS, on a clean `main` at `c9e4592`. | Any session that reddens either has broken something it did not mean to touch. Both are part of the test command (DESIGN §5). |
