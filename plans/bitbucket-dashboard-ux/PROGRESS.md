...
**Status:** in progress — T00 ✅; T01 ✅; T02 ✅; T03 🔍.
**Last updated:** 2026-09-06
**Next `pir-work` will:** review T03 (two-line rows, drop order, separator, emphasis).
...
| T03 | Pure renderer: two-line rows, drop order, separator, emphasis states | T02 | 🔍 | Line one loses [Open]→open zone (col1..pre-button, bb-open) + cyan #id; line two tags·age·branch + right diff, drop branch→+/-→files, dim underline every row; emphasis {verb,state} hover/press; pagination halved. render 235. Deviations: #id cyan (DESIGN 2.7); underline SGR-4-only keeps colours; minus=U+2212 (T04 live-check); cockpit-test paging now 7 pages (FINDINGS). |
