...
| The terminal gestures are **not** agents-only | `terminalCommand` used to return early with no agent attached, on the assumption the fleet list held one repo shell. Nothing else believed it: `terminals` is keyed for the list exactly as for an agent, the strip draws `[+ add]` and `[x]` off that list on every screen, and the footer legends `⌥t` unattached. So the guard bought no simplicity and made every one of those buttons visibly do nothing. A new list terminal opens at `panes.repo` (the cockpit checkout), never at some agent's worktree. |
| Unattached, `⌥[`/`⌥]` always mean **terminals**, whatever holds focus | The routing asks "is the diff pane focused?", but at the fleet list that pane is the welcome/notes display, not a revdiff — there is no mode to cycle. Focus sitting there therefore swallowed the keys into a `diffModeCommand` that returns immediately, so terminals could be opened up there and then not switched between. |
...
| `⌥[` / `⌥]` route by **which pane is focused** | The keys append `next`/`prev` to `cmd` unconditionally; the daemon reads the cockpit tab's active pane (`is_active`) and sends them to the diff-mode switch when the **diff** pane holds focus **and an agent is attached**, to the terminal cycler otherwise (see the unattached row above). `⌥t`/`⌥w` are always terminals. |
...
| `agenda add <slug>` on a calendar that already exists is a **repair**, not a refusal | DESIGN §2.7 prints the fixing command *on* the loud line — an expired sign-in draws `home  sign-in expired · agenda add home` — and that command used to exit 1 with "already connected". The column named something that turned you away (found by hand in T08). Now it re-signs-in the **account** (what actually broke) and leaves the calendar row untouched, re-fetching every calendar that shared the sign-in so all their lines clear at once. The calendar is **probed first**: a working one still refuses, a `gone` one is sent to `agenda rm`, and a transient failure (wifi off, a 5xx) changes nothing rather than opening a browser that cannot help. |
| That probe makes the **events** call, not just the token call | A grant whose per-scope calendar checkbox was left unticked **refreshes perfectly** and 403s only on events. So a token-only probe answered "the sign-in works" and refused — pointing at `agenda rm`, the one command that row above forbids for a scope failure — while the column was drawing `calendar permission not granted · agenda add work`. The dead end the repair exists to close, surviving in the failure whose fix is hardest to guess. The verdict is read with the model's own `errorKind`, the same function the column classified it with, so the command and the line that sent you to it cannot drift apart. Found in T08's review. |
| `AGENDA_DRY_RUN=1` binds the **repair** too, with its own check | The repair branch is decided before `add` reaches its dry-run block, so a dry run aimed at an already-connected slug opened a real browser and signed in for real — the opposite of what the flag promises, on one of the two seatbelts every hands-on check in this project is handed over under (DESIGN §5.2). |
