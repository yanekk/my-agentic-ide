...
| The mouse is left to the **app**; **Shift** is what selects | claude's TUI (and revdiff) enable mouse reporting, so a left-button binding carrying `mouse_reporting = true` is answered by WezTerm and the app never sees a click at all — which is how copy-on-select, added that way, made everything in the Claude pane unclickable. Stock WezTerm ships an **empty** mouse_reporting table for exactly this reason (`wezterm --config-file /dev/null show-keys` prints no such section), so the fix is to put nothing back into it. Selection rides WezTerm's own escape hatch instead — `bypass_mouse_reporting_modifiers` (Shift): held, the event is not reported to the app, so Shift+drag selects over claude and copies on release. The explicit Shift bindings only make that gesture identical in a plain shell, at the cost of WezTerm's default Shift+click-extends. |
