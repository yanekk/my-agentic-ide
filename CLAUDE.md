...
| claude keeps the **press**; WezTerm takes the **drag** and the **release** | Both halves of claude — the fleet view and an agent session — and revdiff turn on *full* mouse reporting (`?1000h ?1002h ?1003h ?1006h`, motion included), so by default press, drag and release all go to the app: WezTerm makes no selection and a drag over the Claude pane leaves the clipboard empty however it looks on screen. Binding all three under `mouse_reporting = true` bought the copy and lost the clicks — WezTerm answered the press too, so claude never saw one. Splitting the gesture gets both: only `Drag` and `Up` are bound, `Down` is deliberately absent, and keeping the release costs nothing because claude's mouse decoder labels events `press`/`release` and then only ever tests for `press`. The `ClearSelection` paired with the copy on `Up` is the **anchor**, not tidiness: with no Down binding nothing sets the selection origin, so `extend_selection_at_mouse_cursor` falls back to `origin.unwrap_or(<cursor>)` and the first drag event anchors it — but only if no origin is left over, else the next drag rubber-bands from where the previous one started. The visible cost is that the highlight goes as you let go, just after the text lands on the clipboard. Shift remains the full escape hatch (`bypass_mouse_reporting_modifiers`, left at its SHIFT default): it is stripped before the binding lookup, so Shift+drag is a stock terminal selection — highlight kept — and Shift+double/triple-click is what still word/line-selects over claude, rather than stealing more presses. |
