// Assertions for the usage tests, printing in the same shape as the bitbucket,
// agenda and notes harnesses next door so run.sh can count "  ok" / "  FAIL"
// lines across all of them and one eye can read the lot. A copy on purpose: the
// suites share no code (zero-dependency project, no shared test lib), so each
// carries its own tiny asserter rather than reaching across dirs.
